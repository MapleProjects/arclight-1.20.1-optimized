...
public class Unsafe {
    ...
    public static Class<?> defineClass(String s, byte[] bytes, int i, int i1, ClassLoader classLoader, ProtectionDomain protectionDomain) {
        try {
            return defineClass.invokeExact(s, bytes, i, i1, classLoader, protectionDomain);
        }
    ...
    public static Class<?> defineAnonymousClass(Class<?> aClass, byte[] bytes, Object[] objects) {
        if (H_DEF_CLASS != null) {
            try {
                return H_DEF_CLASS.invokeExact(aClass.getClassLoader(), aClass, new ClassReader(bytes).getClassName(), bytes, 0, bytes.length, aClass.getProtectionDomain(), false, 11, objects);
            }
            catch (Throwable t) {
    ...
    static {
        ...
        try {
            handle = Unsafe.lookup().findStatic(ClassLoader.class, "defineClass0", MethodType.methodType(Class.class, ClassLoader.class, Class.class, String.class, byte[].class, Integer.TYPE, Integer.TYPE, ProtectionDomain.class, Boolean.TYPE, Integer.TYPE, Object.class));
        }
        catch (Throwable t) {
            try {
                handle = MethodHandles.dropArguments(Unsafe.lookup().findVirtual(ClassLoader.class, "defineClassInternal", MethodType.methodType(Class.class, Class.class, String.class, byte[].class, ProtectionDomain.class, Boolean.TYPE, Integer.TYPE, Object.class)), 4, Arrays.asList(Integer.TYPE, Integer.TYPE));
            }
            catch (Throwable t2) {
                handle = null;
            }
        ...
        INSTANCE = securityManagerPresent ? new SecurityManagerCallerClass() : new StackWalkerCallerClassimplements();
    }

    private static interface CallerClass {
        public Class<?> getCallerClass();
    }

    private static class StackWalkerCallerClassimplements
    implements CallerClass {
        ...
        @Override
        public Class<?> getCallerClass() {
            return this.walker.walk(s -> ((StackWalker.StackFrame)s.skip(3L).findFirst().orElseThrow()).getDeclaringClass());
        }
    }

    private static class SecurityManagerCallerClass
