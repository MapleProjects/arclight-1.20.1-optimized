...
JsonDeserializer<Item> {
    public Item deserialize(JsonElement element, Type type, JsonDeserializationContext context) throws JsonParseException {
        ...
        return new Item(value.has("id") ? value.get("id").getAsString() : null, count, value.has("tag") ? (ItemTag)context.deserialize(value.get("tag"), ItemTag.class) : null);
    }
    ...
    public JsonElement serialize(Item content, Type type, JsonSerializationContext context) {
        ...
        if (content.getCount() != -1) {
            object.addProperty("Count", (Number)content.getCount());
        }
        if (content.getTag() != null) {
            object.add("tag", context.serialize((Object)content.getTag()));
        }
        return object;
    }
}
